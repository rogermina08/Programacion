...
public class ProductoController implements Serializable {
    ...
    @EJB
    private beans.ProductoFacade ejbFacade;
    ...
    public Producto getSelected() {
        if (current == null) {
            ...
            current.setProductoPK(new persistence.ProductoPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public Producto getProducto(persistence.ProductoPK id) {
    ...
    public static class ProductoControllerConverter implements Converter {
        ...
        persistence.ProductoPK getKey(String value) {
            persistence.ProductoPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new persistence.ProductoPK();
            key.setId(Integer.parseInt(values[0]));
            key.setContenido(Double.parseDouble(values[1]));
            key.setCantidad(Integer.parseInt(values[2]));
            key.setCosto(Double.parseDouble(values[3]));
            return key;
        }

        String getStringKey(persistence.ProductoPK value) {
            StringBuilder sb = new StringBuilder();
            sb.append(value.getId());
            sb.append(SEPARATOR);
            sb.append(value.getContenido());
            sb.append(SEPARATOR);
            sb.append(value.getCantidad());
            sb.append(SEPARATOR);
            sb.append(value.getCosto());
            return sb.toString();
        }
